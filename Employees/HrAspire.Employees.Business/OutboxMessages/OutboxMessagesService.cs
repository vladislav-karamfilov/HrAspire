namespace HrAspire.Employees.Business.OutboxMessages;
...
public class OutboxMessagesService : IOutboxMessagesService
{
    ...
    private (EmployeeDeletedEvent? @Event, string? ErrorMessage) TryDeserializeEmployeeDeletedEvent(
        OutboxMessage message)
    {
        EmployeeDeletedEvent? @event = null;
        string? errorMessage = null;
        try
        {
            @event = JsonSerializer.Deserialize<EmployeeDeletedEvent>(message.Payload);
            if (@event is null)
            {
